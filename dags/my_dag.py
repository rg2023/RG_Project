...
async def fetch_exchange_rates(base_currency="USD") -> ExchangeRateData:
...
def fetch_and_store_exchange_rates(**kwargs):
    ...
    save_to_minio(df.to_csv(index=False).encode("utf-8"))
...
with dag:
    ...
    fetch_and_store >> create_table >> load_clickhouse >> process_and_save